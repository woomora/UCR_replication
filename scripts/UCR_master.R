...
# If you wish to replicate the full data processing pipeline (raw → harmonized),
# including variable cleaning, harmonization, and regional standardization,
# please run: "scripts/UCR_lapop_merge.R"

if(!file.exists("data/derived/UCR_lapop_merge.rds")){
  # Data cleaning and harmonization
  source("scripts/UCR_lapop_merge.R")
}else{
  # Load harmonized and derived dataset
  lapop <- read_rds("data/derived/UCR_lapop_merge.rds")
}
...
lapop_analysis <- lapop |> 
  filter(
    ...
    !is.na(edc)            # Must have years of schooling
  ) |> 
  mutate(
...
if(!file.exists("data/derived/proder.rds")){
  # Data cleaning and harmonization
  source("scripts/UCR_proder.R")
}
